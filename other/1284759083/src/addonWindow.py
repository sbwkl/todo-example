...
class Window(QWidget):
    ...
    def addNote(self, words):
        ...
        for word in words:
            ...
            wd['term'] = note["term"]
            wd['image'] = note["image"] 
            wd['BrEPron'] = note["BrEPron"] 
            wd['AmEPron'] = note["AmEPron"] 
            wd['BrEPhonetic'] = note["BrEPhonetic"]
            wd['AmEPhonetic'] = note["AmEPhonetic"]
            wd['definitions'] = note["definitions"]
            wd['samples'] = note["samples"]
            wd['pron'] = note["pron"]
            wd['order'] = word['order']
            wds.append(wd)
            mw.col.addNote(note)
        ...
        self.log('word list ' + str(wds))
        # back up word list to csv
        with open('youdao-wordlist.csv', 'w', encoding='utf-8') as cfo:
            fns = ['term','image','BrEPron','AmEPron','BrEPhonetic','AmEPhonetic','definitions','samples','pron','order']
